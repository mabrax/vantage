...
status: complete
resolution_date: 2026-07-23T10:29:36-04:00
resolution_commit: 54c4895da40bdefe6bf6cf483b0ebc95265435f5
---
...
**Branch**: main
**Repository**: vantage
**Resolution Date**: 2026-07-23T10:29:36-04:00
**Resolution Commit**: 54c4895da40bdefe6bf6cf483b0ebc95265435f5
...
| --- | --- | --- | --- |
| Repository inventory, conventions, and division of architecture responsibilities | Exhaustive non-`.rpi/` file inventory; `.gitignore`; `README.md`; `docs/README.md`; all four named architecture documents; resolution-time inventory and targeted search | full | Resolved: the non-`.rpi/` tree contains only `.gitignore` and eight Markdown documents; no implementation or test evidence exists. |
| Deno and Codex versions, releases, installation, and compatibility | Local `command -v`/`--version`; Deno 2.9.3 release and installation docs; Codex 0.145.0 release, CLI, and app-server docs; resolution-time pin and compatibility search | full | Resolved repository fact: no selected pair, pin, range, compatibility output, or packaged validation exists. The actual pair remains an explicit assumption because selection and runtime validation are external to static evidence. |
| Codex TypeScript and JSON Schema generators | Local 0.145.0 help and clean generation; stable and experimental generated unions; official version-specific generator contract; resolution-time artifact search | full | Resolved: no generated TypeScript, JSON Schema, generator metadata, or coverage manifest is committed. |
| App-server lifecycle and wire contracts | Version-tagged 0.145.0 app-server documentation, generated protocol unions, Vantage architecture, one observed 0.145.0 session, and resolution-time transcript search | full | Resolved repository fact: no replayable raw transcript exists. Availability of unrecoverable live-session payloads is recorded as an assumption. |
| Real authenticated text-turn events and coverage classification | One ephemeral authenticated 0.145.0 text turn, stable generated notification union, official item lifecycle reference, and resolution-time manifest search | full for current evidence | Resolved repository fact: no transcript or method/event coverage manifest exists. Occurrence of optional notifications remains an explicit runtime assumption. |
| Deno subprocess and process-tree behavior | Deno API docs, Deno 2.9.3 implementation/tests, current Desktop lifecycle docs, Vantage reliability contract, and resolution-time platform-evidence search | full for documented behavior | Resolved web fact: primary Deno documentation promises direct-process signaling, not portable descendant-tree cleanup. No OS set or packaged validation exists; platform cleanup remains an explicit runtime assumption. |
| Environment diagnostics and testing evidence | Local missing-Deno observation, Deno missing-executable tests, Codex account behavior, exhaustive repository test inventory, documented test strategy, and resolution-time implementation search | full | Resolved repository fact: no version gate, diagnostics, harness, measurements, tests, or concrete unsupported-version contract exists. The future error shape and state transition remain an explicit product/implementation assumption. |
...
Resolution at commit `54c4895da40bdefe6bf6cf483b0ebc95265435f5` closed seven unique repository-answerable gaps as confirmed absences and one web-answerable gap as a documented absence of a portable Deno guarantee. No web-answerable gaps remain unresolved. Five runtime or product-dependent gaps are recorded as explicit assumptions.
...
A resolution-time inventory at the resolution commit returned the same nine-file shape. A targeted search for version pins, generated schemas, manifests, transcripts, process-tree results, implementation, diagnostics, fixtures, measurements, leak checks, and compatibility evidence found only requirement text in the architecture documents. This confirms the recorded material absences without expanding the research scope.
...
Resolution-time repository search confirms that this remains the complete repository fact: neither a selected pair nor evidence validating one is present. Choosing a pair and validating it in packaged runtime conditions cannot be recovered from repository or public documentation, so that fact is represented below as an assumption rather than inferred.
...
Resolution-time repository search confirms that generated TypeScript, JSON Schema, generator metadata, and a method/event coverage manifest are all absent. The architecture's references to those artifacts remain requirements, not committed artifacts.
...
Resolution-time repository search confirms both absences. The unretained full payload stream from the prior live session is not reconstructable from its summaries, and optional-notification occurrence cannot be generalized beyond the observed runtime conditions; both facts are recorded below as assumptions.
...
The resolution-time primary-source check reached the same bounded fact. The current [`Deno.ChildProcess.kill()` documentation](https://docs.deno.com/api/deno/subprocess/) says it kills the process, documents direct-child status and detached children that may outlive the parent, and does not state descendant-tree semantics. The current [Deno Desktop window lifecycle](https://docs.deno.com/runtime/desktop/windows/#closing-the-app) describes when the runtime exits, but does not add a descendant-cleanup guarantee. This resolves the web-answerable documentation gap; actual per-platform cleanup remains runtime-only evidence.
...
Resolution-time search confirms there is still no source, process host, version gate, error mapper, fake child/server, temporary-repository fixture, isolated authenticated-test runner, timing collector, leak checker, CI job, or executable unsupported-version behavior. A future unsupported-version error shape and state transition therefore cannot be described as a current repository fact.
...
Every gap from the original `Evidence Gaps` section and every non-empty or below-full `Coverage And Gaps` row was normalized and classified. Repeated statements across those two sections were counted once.

| Class | Unique gaps | Resolution |
| --- | ---: | --- |
| Repository-answerable | 7 | Resolved as confirmed absences: implementation/tests; selected or validated version policy; committed generated protocol artifacts; repository transcript/manifest; selected OS set and packaged results; executable diagnostics/test infrastructure; concrete unsupported-Codex behavior. |
| Web-answerable | 1 | Resolved from primary Deno sources: the public subprocess and Desktop lifecycle contracts do not promise portable descendant-tree cleanup. |
| External | 5 | Converted to the explicit assumptions below: validated pair, replayable live transcript, optional-event occurrence, per-OS descendant cleanup, and unsupported-version behavior. |

No second repository-research round was needed because the first targeted inventory and search resolved every repository-answerable gap. No web-answerable gap remains unresolved.

## Evidence Gaps

1. Assumption: Later design work treats the Deno/Codex compatibility pair as unset and unvalidated — unverifiable because selecting a pair and producing packaged compatibility results require a product decision and runtime validation that are absent from the repository and public documentation.
2. Assumption: Later design work treats no replayable schema-valid authenticated transcript as available — unverifiable because the prior bounded session's complete raw payloads were not retained and cannot be reconstructed from ordered envelope summaries.
3. Assumption: Later design work does not require any optional notification to occur in a text turn — unverifiable because notification occurrence depends on the live model, account, configuration, integrations, tools, policies, and CLI behavior for that run.
4. Assumption: Later design work treats no operating system as validated for complete Codex descendant cleanup — unverifiable because the initial supported OS set has not been selected and no packaged per-platform process-tree measurements exist.
5. Assumption: Later design work treats the unsupported-Codex error shape and state transition as unspecified — unverifiable because they depend on a Vantage version policy and executable implementation that do not exist.
